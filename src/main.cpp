...
#if defined(NDEBUG)
# error "Glocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfe150964c6f1e142d7cf289054634e354e5fe5fd48e681808695f9a3d811b2b0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Glocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Glocoin Signed Message:\n";
...
{
    // Glocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Glocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Glocoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Glocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Glocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Glocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x233738d95112abaa32bf7c7fe15cdf5d307eb615174f1c66c20c4ff7016e79a6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Glorious System Times";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0403e34a7b58d8748ca2036598e095598a7868b34c5c81bc3c0541e3539c625c05b0627e94d4d598369fe4b6029436807661d5eaa2f947d3619b4bef9208465eba") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1529900429;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085500791;

        if (fTestNet)
        {
            block.nTime    = 1529900395;
            block.nNonce   = 386033552;
        }
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x753b7615c3e7cc59125200f50febe1a68b032de958b885a592b4441c025ba741"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Glocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GlocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GlocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GlocoinMiner(CWallet *pwallet)
{
    printf("GlocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("glocoin-miner");
    ...
    try { loop {
        ...
        printf("Running GlocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GlocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GlocoinMiner, pwallet));
}
